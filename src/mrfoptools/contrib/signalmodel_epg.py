"""
Direct copy of Tom Grieslers public implementation of the Extended Phase Graph (EPG) signal model.
...
def simulate_fisp_part(t1: List[float], t2: List[float], beats: int, shots: int, fa: Union[np.ndarray, torch.tensor], tr: Union[np.ndarray, torch.tensor], ph: Union[np.ndarray, torch.tensor], prep: List[int], ti: List[int], t2te: List[int], te: float, inv_eff: float=1., delta_B1: float=1.):
    """
    Function to calculate the orthogonality cost function for a number of tissues for a given MRF sequence using Boch/isochromat summation. 
