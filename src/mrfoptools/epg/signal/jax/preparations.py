...
def prepare_inversion_omega(T1: float,
    ...
    """
    omega = epgjax.r_epg(T1, T2, TI) @ inversion_operator @ prepare_equilibrium_omega(M0, max_states)
    omega = omega.at[2, 0].set(omega[2, 0] + M0 * epgjax.b_epg(T1, TI))
    return omega
...
def prepare_T2_omega(T2: float,
    ...
    """
    omega = epgjax.preparation_T2_op(T2, T2_prep_time) @ prepare_equilibrium_omega(M0, max_states)
    return omega
