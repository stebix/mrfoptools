...
def main():
    ...
    initializations_list = [yun_canonical_initializations, const_initializations]
    ...
    for i, initializations in enumerate(tqdm.tqdm(initializations_list, unit='subruns')):
        ...
        expdir = Path(
            f'/data/42-julia-hpc-physik-jakob/jas05ha/data/mrf-optruns-stable-archer/{sweep_ID}/subrun-{subrun_index}'
        )
        expdir.mkdir(exist_ok=True, parents=True)

        runinfo = RunInfo(
            base_name='tradspecies-conFIG-initsweep',
            run_dir=expdir,
            tags=['init-sweep', 'conFIG', 'tradspecies'],
            seed=1337,
